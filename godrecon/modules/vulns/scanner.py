...
class VulnerabilityModule(BaseModule):
    ...
    async def _execute(self, target: str, config: Config) -> ModuleResult:
        ...
        _scan_monotonic_start = time.monotonic()
        if notifier:
        ...
        if notifier:
            await self._send_telegram_alerts(
                ...
                pattern_findings=all_pattern_findings,
                config=config,
                scan_start_time=_scan_monotonic_start,
    ...
    async def _send_telegram_alerts(
        ...
        await notifier.alert_scan_finished(
            ...
            stats={
                ...
                "targets_scanned":  len(all_targets) if all_targets else 1,
                "total_cves":       len(cve_findings),
                "total_vulns":      len(pattern_findings),
