...
from godrecon.utils.telegram_notifier import get_notifier_from_config
...
class SubdomainAggregator(BaseModule):
    ...
    async def _execute(self, target: str, config: Config) -> ModuleResult:
        ...
        if sub_cfg.permutation.enabled and all_subdomains:
            ...
            try:
                # Cap known subs fed to permutation to avoid 200k+ candidate explosion
                known_capped = set(list(all_subdomains.keys())[:2000])
                async with PermutationScanner(
                    nameservers=config.dns.resolvers,
                    timeout=config.dns.timeout,
                ) as scanner:
                    perm_results = await asyncio.wait_for(
                        scanner.run(
                            domain=target,
                            known=known_capped,
                        ),
                ...
                logger.warning("Permutation scan timed out after 60s — skipping")
            except Exception as exc:
        ...
        if discovered_list:
            ...
            try:
                async with AsyncDNSResolver(
                    ...
                    is_wildcard = await asyncio.wait_for(
                        resolver.detect_wildcard(target), timeout=30
                    )
                    if is_wildcard:
                        logger.info(
                    ...
                    resolved_ips = await asyncio.wait_for(
                        resolver.bulk_resolve(discovered_list, "A"), timeout=120
                    )
            except asyncio.TimeoutError:
                logger.warning("DNS resolution timed out for %s — continuing with unresolved subdomains", target)
                resolved_ips = {}
            except Exception as exc:
        ...
        # ── Telegram: subdomain discovery alert ─────────────────────
        try:
            notifier = get_notifier_from_config(config)
            if notifier:
                await notifier.alert_subdomains_found(
                    target=target,
            ...
            logger.error("Telegram subdomain alert failed: %s", _tg_exc)
